...
class AppController {
  constructor() {
	...
	//this._API_KEY = '488354959095';
	//proyecto : prueba-notify ------>el primer key
	this._API_KEY = 'AIzaSyDIv0TBMTPrpOPJfLagVOEnTvwCyT0rGyA';
	//this._API_KEY = 'AIzaSyABOkloC1viQxQrs826J9wMH44vSPVhEY0';
	//actualmente funciona con el 4to:
//this._API_KEY = 'AIzaSyCiuXqn-wW_mdpoRivfzOgMTyogChANW9E';
    ...
    if ('serviceWorker' in navigator) {
      ...
      .catch(err => {
        this.showErrorMessage(
          'Atención: Para habilitarlo, debes',
          'ingresar a https://www.livees.net.'
        );
        console.error(err);
      });
    } else {
      this.showErrorMessage(
        'No soportado para tu navegador',
        'Prueba en Chrome'
      );
    }
  }
  ...
  _stateChangeListener(state, data) {
    ...
    switch (state.id) {
      case 'ERROR':
        this.showErrorMessage(
          'Debes habiltarlo desde https://www.livees.net',
          data
        );
        break;
      default:
  ...
  updatePushInfo() {
    ...
    .then(results => {
	  ...
	  if (this._currentSubscription.endpoint.indexOf(
        ...
        infoFunction = () => {
          return this.getFCMInfo(this._currentSubscription, payload,
            this._API_KEY);
        };
      }
  ...
  getGCMInfo(subscription, payload, apiKey) {
	...
	document.getElementById("id_dev").value = subscriptionId;
	document.getElementById("id_dev_bkp").value = subscriptionId;
    const gcmAPIData = {
      to: subscriptionId
    };
   ...
   getFCMInfo(subscription, payload, apiKey) {
    const headers = {};
  ...
  sendPushMessage(subscription, payloadText) {
    ...
    .then(results => {
      ...
      if (subscription.endpoint.indexOf(
        ...
        'https://fcm.googleapis.com/fcm/send') === 0) {
        infoFunction = () => {
          return this.getFCMInfo(subscription, payload,
            this._API_KEY);
        };
